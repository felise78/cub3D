...
int	copy_file(t_cub *cub3D)
{
	...
	cub3D->file->cub_file = (char *)ft_calloc(sizeof(char), cub3D->file->file_len + 1);
...
int	check_file_pollution(t_cub *cub3D)
{
	...
	while (cub3D->file->file_grid[i])
	{
		...
		{
			free_all(cub3D, 0, 0);
			return (ft_dprintf(2, "Error: Invalid .cub file\n"), 1);
		}
		i++;
	}
	return (0);
